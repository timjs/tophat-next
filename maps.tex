...
\newmacro{O-Picks}{
  \begin{function}
    ...
    % \Picks(\Update b)                          &=& \nothing \\
    % \Picks(\View b)                            &=& \nothing \\
    \Picks(\Pick \set{\more{L_i \mapsto t_i}}) &=& \set{L^n_i \mid \lnot\Failing(t_i), i \in I} \\
  \addlinespace
    % \Picks(\Done b)                            &=& \nothing \\
    % \Picks(t_1 \Pair t_2)                      &=& \nothing \\
...
\newmacro{O-Inputs}{
  ...
  \addlinespace
    ...
    \Inputs(t_1 \Step e_2,\sigma)                     &=& \Inputs(t_1,\sigma) \cup \set{\IContinue L_i \mid \Value(t_1,\sigma) = v_1, e_2\ v_1 \eval t_2, L^n_i \in \Picks(t_2) } \\
  ...
  % \begin{function}
    % \signature{\sem{\cdot} : \mathrm{Restricted tasks} \to \mathrm{Expressions}} \\
      ...
      \sem{\When \set{\more{e_i \mapsto s_i}}}
        &=& \If{e_1}{\sem{s_1}}{\Fail} \Choose \ldots \Choose \If{e_n}{\sem{s_n}}{\Fail} \\
        % &=& \If{e_1}{\sem{s_1}}{\ldots \If{e_n}{\sem{s_n}}{\Fail}}\\
      \sem{\Pick \set{\more{S_i \shortmid e_i \mapsto s_i}}}
        &=& \Enter\Label\set{\more{S_i}} \Step \lambda x. \sem{\When \set{\more{x \equiv S_i \land e_i \mapsto s_i}}} \\
