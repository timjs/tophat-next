% !TEX root=main.tex


%% Language %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newmacro{G-Types}{
  \begin{grammar}
    Types
      & \tau &::= & \tau_1 \to \tau_2    & – function type \\
      &      &\mid& \Reference \tau      & – reference type \\
      &      &\mid& \Task \tau           & – task type \\
      &      &\mid& \rho                 & – record types \\
      &      &\mid& \pi                  & – primitive types \\
    \addlinespace
    Record types
      & \rho &::= & \tuple{\more{x = \tau_x}} & – record type \\
    \addlinespace
    Primitive types
      & \pi  &::= & \Bool                & – boolean type \\
      &      &\mid& \Int                 & – integer type \\
      &      &\mid& \String              & – string type \\
      &      &\mid& \Label\set{\more{S_i}} & – label type \\
    \addlinespace
    Basic types
      & \beta &::= & \tuple{\more{x = \beta_x}} & – record type \\
      &       &\mid& \pi                    & – primitive type \\
  \end{grammar}
}

\newmacro{G-Expressions}{
  \begin{grammar}
    Expressions
      & e    &::= & \lambda m:\tau.\ e   & – abstraction \\
      &      &\mid& e_1\ e_2             & – application \\
      &      &\mid& x                    & – variable \\
      &      &\mid& l                    & – location \\
    \addlinespace
      &      &\mid& \If{e_1}{e_2}{e_3}   & – conditional \\
      &      &\mid& \tuple{\more{x = e_x}} & – record \\
    \addlinespace
      &      &\mid& c                    & – constant \\
      &      &\mid& p                    & – pretask \\
    \addlinespace
    Constants
      & c    &::= & B                    & – boolean \\
      &      &\mid& I                    & – integer \\
      &      &\mid& S                    & – string \\
  \end{grammar}
}

\newmacro{G-Pretasks}{
  \begin{grammar}
    Pretasks
      & p    &::= & \Enter \beta         & – unvalued editor \\
      &      &\mid& \Update e            & – valued editor \\
      &      &\mid& \View e              & – valued read-only editor \\
    \addlinespace
      &      &\mid& e_1 \Pair e_2        & – pairing \\
      &      &\mid& \Done e              & – internal value \\
      &      &\mid& e_1 \Choose e_2      & – choice \\
      &      &\mid& \Fail                & – fail task \\
    \addlinespace
      &      &\mid& e_1 \Trans e_2       & – transform \\
      &      &\mid& e_1 \Step e_2        & – step \\
      &      &\mid& \Forever e           & – forever running task \\
    \addlinespace
      &      &\mid& \New\ e              & – new shared value \\
      &      &\mid& e_1 := e_2           & – assignment \\
      &      &\mid& \Change e            & – shared editor \\
      &      &\mid& \Watch e             & – shared read-only editor \\
  \end{grammar}
}

\newmacro{G-Values}{
  \begin{grammar}
    Values
      & v    &::= & \lambda m:\tau.\ e   & – abstraction \\
      &      &\mid& l                    & – location \\
    \addlinespace
      &      &\mid& \tuple{\more{x = v_x}} & – record value \\
    \addlinespace
      &      &\mid& c                    & – constant \\
      &      &\mid& t                    & – task \\
    \addlinespace
    Basic values
      & b    &::= & \tuple{\more{x = b_x}} & – record basic value \\
      &      &\mid& c                    & – constant \\
  \end{grammar}
}

\newmacro{G-Tasks}{
  \begin{grammar}
    Tasks
      & t    &::= & \Enter \beta         & – unvalued editor \\
      &      &\mid& \Update b            & – valued editor \\
      &      &\mid& \View b              & – valued read-only editor \\
    \addlinespace
      &      &\mid& t_1 \Pair t_2        & – pairing \\
      &      &\mid& \Done v              & – internal value \\
      &      &\mid& t_1 \Choose t_2      & – choice \\
      &      &\mid& \Fail                & – fail task \\
    \addlinespace
      &      &\mid& e_1 \Trans t_2       & – transform \\
      &      &\mid& t_1 \Step e_2        & – step \\
      &      &\mid& \Forever t           & – forever running task \\
    \addlinespace
      &      &\mid& \Change l            & – shared editor \\
      &      &\mid& \Watch l             & – shared read-only editor \\
  \end{grammar}
}

\newmacro{G-Matches}{
  \begin{grammar}
    Matches
      & m    & ::=& \_                     & – ignore \\
      &      &\mid& x                      & – capture \\
      &      &\mid& \tuple{\more{x = m_x}} & – rename \\
      &      &\mid& \tuple{..}             & – unpack \\
  \end{grammar}
}

\newmacro{G-Inputs}{
  \begin{grammar}
    Inputs
      & i    & ::=& \IEnter b            & – enter basic value \\
      &      &\mid& \IContinue S         & – continue with label \\
      &      &\mid& \IPick S             & – pick label \\
  \end{grammar}
}

\newmacro{G-Extensions}{
  \begin{grammar}
    Types
      & \tau &::= & \ldots               & type \\
      &      &\mid& \List \tau           & – list type\\
    Basic types
      & \beta &::= & \ldots              & \\
      &       &\mid& \List \beta         & – list type\\
    \addlinespace
    Expressions
      & e    &::= & \ldots               & \\
      &      &\mid& z                    & – symbol \\
    \addlinespace
      &      &\mid& u\ e_1               & – unary operation \\
      &      &\mid& e_1\ o\ e_2          & – binary operation \\
    \addlinespace
      &      &\mid& [\ ]_\tau            & – nil \\
      &      &\mid& e_1 :: e_2           & – cons \\
      &      &\mid& \Head e              & – list head \\
      &      &\mid& \Tail e              & – list tail \\
    \addlinespace
    Values
      & v    &::= & \ldots               & \\
      &      &\mid& z                    & – symbol \\
      &      &\mid& u\ v                 & – symbolic unary operation \\
      &      &\mid& v_1\ o\ v_2          & – symbolic binary operation \\
      &      &\mid& [\ ]_\beta           & – nil \\
      &      &\mid& v_1 :: v_2           & – cons value \\
    Basic values
      & b    &::= & \ldots               & \\
      &      &\mid& [\ ]_\beta           & – nil \\
      &      &\mid& b_1 :: b_2           & – cons value \\
    \addlinespace
    Symbolic inputs
      & \tilde{i}& ::=& \IEnter z        & – enter symbol \\
      &      &\mid& \IContinue S         & – continue with label \\
      &      &\mid& \IPick S             & – pick label \\
  \end{grammar}
}

\newmacro{G-Predicates}{
  \begin{grammar}
    Predicates
      & \phi &::= & c                      & – constant \\
      &      &\mid& z                      & – symbol \\
      % &      &\mid& \unit                  & – unit \\
      &      &\mid& \tuple{\more{x = \phi_x}} & – records \\
      &      &\mid& [\ ]_\beta             & – nil \\
      &      &\mid& \phi_1 :: \phi2        & – cons \\
      &      &\mid& u\ \phi                & – symbolic unary operation \\
      &      &\mid& \phi_1\ o\ \phi_2      & – symbolic binary operation \\
  \end{grammar}
}

\newmacro{G-Operations}{
  \begin{grammar}
    Unary operations
      & u    &::= & \lnot                & – not \\
      &      &\mid& -                    & – negate \\
      &      &\mid& \Len                 & – length \\
      &      &\mid& \Uniq                & – unique \\
    \addlinespace
    Binary operations
      & o    &::= & \land \Mid \lor                                     & – logical \\
      &      &\mid& < \Mid \le \Mid \equiv \Mid \nequiv \Mid \ge \Mid > & – equational \\
      &      &\mid& + \Mid - \Mid \times \Mid /                         & – numerical \\
      &      &\mid& \pp \Mid \in                                        & – append, element \\
  \end{grammar}
}

\newmacro{G-Restricted}{
  \begin{grammar}
    Statements
      & s    &::= & m \Gets r; s         & – bind \\
      &      &\mid& r                    & – execute \\
    \addlinespace
    Restricted tasks
      & r    &::= & \Enter S\ \beta      & – unvalued editor \\
      &      &\mid& \Update S\ e         & – valued editor \\
      &      &\mid& \View S\ e           & – valued read-only editor \\
      &      &\mid& \Change S\ l         & – shared editor \\
      &      &\mid& \Watch S\ l          & – shared read-only editor \\
      &      &\mid& \Done e              & – internal value \\
    \addlinespace
      &      &\mid& \Pair   \set{\more{s_i}}                            & – pairing \\
      &      &\mid& \Choose \set{\more{s_i}}                            & – alternating \\
      &      &\mid& \When   \set{\more{e_i \mapsto s_i}}                & – system choice \\
      &      &\mid& \Pick   \set{\more{S_i \shortmid e_i \mapsto s_i}}  & – user choice \\
    \addlinespace
      &      &\mid& x_0\ \tuple{\more{x = e_x}} & – application \\
      &      &\mid& \New\ e                  & – new shared value \\
      &      &\mid& l := e                   & – assignment \\
  \end{grammar}
}

\newmacro{G-Derived}{
  \begin{equations}
      \Let{m = e_1}{e_2}
        &:=& (\lambda m. e_2)\ e_1 \\
      e.x
        &:=& \Let{\tuple{x = x'} = e}{x'} \hint{where $x'$ is fresh} \\
    \addlinespace
      \Pair \set{\more{e_i}}
        &:=& e_1 \Pair\ (e_2 \Pair\ (\ldots \Pair e_n)) \hint{for $n \ge 2$} \\
      \Choose \set{\more{e_i}}
        &:=& e_1 \Choose\ (e_2 \Choose\ (\ldots \Choose e_n)) \hint{for $n \ge 2$} \\
    \addlinespace
      \When \set{\more{e_i' \mapsto e_i}}
        &:=& \If{e_1'}{e_1}{\Fail} \Choose \ldots \Choose \If{e_n'}{e_n}{\Fail} \\
      \Pick \set{\more{S_i \mapsto e_i}}
        &:=& \Enter\Label\set{\more{S_i}} \Step \lambda x. \When \set{\more{x \equiv S_i \mapsto e_i}} \\
      \Pick \set{\more{S_i \shortmid e_i' \mapsto e_i}}
        &:=& \Pick \set{\more{S_i \mapsto \If{e_i'}{e_i}{\Fail}}} \\
        &\cong& \Enter\Label\set{\more{S_i}} \Step \lambda x. \When \set{\more{x \equiv S_i \land e_i' \mapsto e_i}} \\
  \end{equations}
}
